...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const allUnfiltered = items.slice().reverse().map((s) => JSON.parse(s));

  const q = ((req.query.q as string) || '').trim().toLowerCase();
  const all = q
    ? allUnfiltered.filter((r) => JSON.stringify(r).toLowerCase().includes(q))
    : allUnfiltered;

  const pageSize = Math.min(Math.max(parseInt(req.query.pageSize as string, 10) || 20, 1), 200);
  const page = Math.max(parseInt(req.query.page as string, 10) || 1, 1);
  const total = all.length;
  const allTotal = allUnfiltered.length;
  const start = (page - 1) * pageSize;
  const out = all.slice(start, start + pageSize);

  res.status(200).json({ total, allTotal, page, pageSize, items: out });
}
